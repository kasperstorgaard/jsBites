(function (window) {
    ...
    function showGreet() {
        return 'welcome to js console (*~*)';
    }

    function load(url) {
        if (navigator.onLine) {
            if (arguments.length > 1 ||  url.indexOf('.js') !== -1 || _.contains(libraries, url)) {
    ...
    function loadScript() {
        var scripts = _.isArray(arguments[0]) ? arguments[0] : Array.prototype.slice.call(arguments);

        return _.map(scripts, function(ref) {
            if (_.isString(ref) || !ref.url) {
                var name = _.isString(ref) ? ref : ref.name;
                ref = _.find(libraries, { 'name': name });
            }
            ...
            var handleLoaded, handleError;
            ref._promise = new Promise(function(resolve, reject) {
                ref.load = function() {
                    return insertScript(ref.url);
                }
                handleLoaded = function() {
                    resolve();
                    window.top.info('Loaded ' + ref.url, 'http://' + window.location.hostname);
                }, handleError = function() {
                    reject();
                    log('Failed to load ' + ref.url, 'error');
                }
            });
    ...
    function codeComplete(event) {
        var cmd = consoleInput.textContent.split(/[;\s]+/g).pop(),
            parts = cmd.split('.'),
        ...
        if (cmd) {
            ...
            if (props.length) {
                ...
                if (ccPosition === false) {
                    ...
                    if (!consoleInput.nextSibling) {
                        cc = document.createElement('span');
                        cc.className = 'suggest';
                        exec.appendChild(cc);
                    }

                    consoleInput.nextSibling.innerHTML = props[ccPosition];
        ...
        if (ccPosition === false && consoleInput.nextSibling) {
            removeSuggestion();
        }
    ...
    function removeSuggestion() {
        ...
        if (enableCC && consoleInput.nextSibling) consoleInput.parentNode.removeChild(consoleInput.nextSibling);
    }
    ...
    var exec = document.getElementById('exec'),
        ...
        libraries = [
            ...
            {
                name: 'dojo',
                url: 'http://ajax.googleapis.com/ajax/libs/dojo/1/dojo/dojo.xd.js'
            },
            {
                name: 'mootools',
            ...
            {
                name: 'rightjs',
                url: 'http://rightjs.org/hotlink/right.js'
            },
            {
                name: 'coffeescript',
    ...
    if (enableCC) {
        exec.parentNode.innerHTML = '<div autofocus id="exec" autocapitalize="off" spellcheck="false"><span id="console-input" spellcheck="false" autocapitalize="off" autocorrect="off"' + (iOSMobile ? '' : ' contenteditable') + '></span></div>';
    ...
    function whichKey(event) {
        var keys = {
            38: 1, 40: 1, Up: 38, Down: 40, Enter: 10, 'U+0009': 9, 'U+0008': 8, 'U+0190': 190, 'Right': 39,
    ...
    function setConsoleInputTo(str) {
    ...
    if (enableCC) {
        // disabled for now
        consoleInput.__onpaste = function (event) {
            setTimeout(function () {
                ...
                consoleInput.innerHTML = consoleInput.innerText;
        ...
        if (keys[which]) {
            if (event.shiftKey) {
                ...
                if (enableCC && window.getSelection) {
                    ...
                    var cursorPos = findNode(selObj.anchorNode.parentNode.childNodes, selObj.anchorNode) + selObj.anchorOffset;
            ...
            if (ccPosition !== false && which == 9) {
                ...
                return false;
            } else if (ccPosition !== false && consoleInput.nextSibling) {
                removeSuggestion();
            }
        }
    };
    ...
    if (enableCC && iOSMobile) {
        ...
        fakeInput.onkeyup = function (event) {
            consoleInput.innerHTML = cleanse(this.value);
            var which = whichKey(event);
            if (enableCC && which != 9 && which != 16) {
                clearTimeout(codeCompleteTimer);
                codeCompleteTimer = setTimeout(function () {
        ...
        form.addEventListener('touchstart', function (event) {
            ...
            if (ccPosition !== false) {
                ...
                if (taps === 2) {
                    completeCode();
                    fakeInput.value = consoleInput.textContent;
                    removeSuggestion();
                    fakeInput.focus();
                } else {
    ...
    function completeCode(focus) {
        ...
        if (document.createRange) {//Firefox, Chrome, Opera, Safari, IE 9+
            ...
            range.selectNodeContents(consoleInput);//Select the entire contents of the element with the range
            ...
            range.moveToElementText(consoleInput);//Select the entire contents of the element with the range
            range.collapse(false);//collapse the range to the end point. false means collapse to end rather than the start
    ...
    window._console.loadScript = loadScript;
    window._console.setConsoleInputTo = setConsoleInputTo;
